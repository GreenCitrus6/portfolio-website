...
function SocialLink( {snsInfo} ) {
  
    return (
      <div className="mx-4 h-6">
        <a className="flex" href={snsInfo.url} target="_blank" title={snsInfo.name}>
          {snsInfo.icon} 
        </a>
      </div>
    )
...
export default function Header() {
  ...
  const githubInfo = {
    name: 'Github',
    icon: <svg className="w-6 h-6 text-gray-50 hover:text-white
    active:text-gray-200
    ease-out duration-200" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
    ...
    url: "https://github.com/GreenCitrus6"
  }
  const linkedInInfo = {
    name: "LinkedIn",
    icon: <svg className="w-6 h-6 text-gray-50 hover:text-white
      active:text-gray-200
      ease-out duration-200" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 15 15">
    ...
    url: "https://www.linkedin.com/in/daniel-aadland-643592298/"
  }
    ...
    return (
        ...
        flex justify-between items-center overflow-hidden z-20">
            ...
            <nav className="flex flex-row justify-center items-center pr-2">
                <SocialLink snsInfo={githubInfo} />
                <SocialLink snsInfo={linkedInInfo} />
            </nav>
        </header>
    )
}